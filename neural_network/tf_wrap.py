...
class tf_model_top:
    ...
    def __init__( self, data_shape, target_shape, tf_prediction_func, tf_optimize_func, tf_error_func, arg = None,\
                 dtype_data = tf.float32, dtype_target = tf.float32 ):
        # tensorflow style data and target defination, as inputs to model
        self.data       = tf.placeholder(dtype_data, data_shape) #tf.placeholder(tf.float32, data_shape) # e.g. [None, 784]
        self.target     = tf.placeholder(dtype_target, target_shape) #tf.placeholder(tf.float32, target_shape) # e.g. [None, 10]
