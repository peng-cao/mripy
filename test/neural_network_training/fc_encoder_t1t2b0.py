...
def test():
    ...
    W_fc1 = weight_variable([2*960, wide_fc])  #40*48 *64 /32
    b_fc1 = bias_variable([wide_fc])    
    ...
    W_fc2 = weight_variable([wide_fc, wide_fc//2])  #40*48 *64 /32
    b_fc2 = bias_variable([wide_fc//2])    
    ...
    #weighting and bias for a layer with 1024 neurons
    W_fc3 = weight_variable([wide_fc//2, wide_fc//4])  #40*48 *64 /32
    b_fc3 = bias_variable([wide_fc//4])    

    # densely connected layer with relu output
    h_fc3 = tf.nn.relu(tf.matmul(h_fc2, W_fc3) + b_fc3)    

    #############dense connection layer 4
    #weighting and bias for a layer with 1024 neurons
    W_fc4 = weight_variable([wide_fc//4, wide_fc//8])  #40*48 *64 /32
    b_fc4 = bias_variable([wide_fc//8])    
    ...
    #keep_prob = tf.placeholder(tf.float32)
    #h_fcn_drop = tf.nn.dropout(h_fc4, keep_prob)    

    W_fcn = weight_variable([wide_fc//8, 4])
    b_fcn = bias_variable([4])    

    #fully connected layer; changed tf.argmax to tf.sigmoid, could also try tf.tanh and tf.nn.relu (not work)
    y_conv = tf.sigmoid(tf.matmul(h_fc4, W_fcn) + b_fcn)    
    ...
    for i in range(200000):
        ...
        if i%10 == 0:
            train_error = accuracy.eval(feed_dict={x: batch_xsnoise, y_: batch_ys, keep_prob: 1.0})
