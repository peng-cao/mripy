...
def tf_optimize_func( model ):
  ...
  # l2-norm
    ...
    loss =  tf.reduce_sum(tf.pow(tf.subtract(model.target[:,0],model.prediction[:,0]),2) ) \
          ...
          + tf.reduce_sum(tf.pow(tf.subtract(model.target[:,4],model.prediction[:,4]),2) ) \
          + tf.reduce_sum(tf.pow(tf.subtract(model.target[:,5],model.prediction[:,5]),2) ) \
          + tf.reduce_sum(tf.pow(tf.subtract(model.target[:,6],model.prediction[:,6]),2) ) 
...
def test1():
    ...
    t1t2_group[0,0] = 1600.0/5000.0
    t1t2_group[0,1] = 70.0/500.0
    t1t2_group[1,0] = 2200.0/5000.0
    t1t2_group[1,1] = 170.0/500.0     
    t1t2_group[2,0] = 1000.0/5000.0
    t1t2_group[2,1] = 20.0/500.0 
    ...
    for i in range(1000000):
        ...
        for k in range(batch_size):
            if batch_ys_tmp[k] <= 4 and batch_ys_tmp[k] > 3:
                batch_ys[k,0] = t1t2_group[0,0] + np.random.uniform(-0.05,0.05)
                batch_ys[k,1] = t1t2_group[0,1] + np.random.uniform(-0.05,0.05)
                batch_ys[k,4] = 1.0
                batch_ys[k,5] = 0.0
                batch_ys[k,6] = 0.0
            elif batch_ys_tmp[k] <= 3 and batch_ys_tmp[k] > 2:
                batch_ys[k,0] = t1t2_group[1,0] + np.random.uniform(-0.05,0.05)
                batch_ys[k,1] = t1t2_group[1,1] + np.random.uniform(-0.05,0.05)   
                batch_ys[k,4] = 0.0
                batch_ys[k,5] = 1.0
                batch_ys[k,6] = 0.0                            
            elif batch_ys_tmp[k] <= 2 and batch_ys_tmp[k] > 1:
                batch_ys[k,0] = t1t2_group[2,0] + np.random.uniform(-0.5,0.5)
                batch_ys[k,1] = t1t2_group[2,1] + np.random.uniform(-0.5,0.5) 
                batch_ys[k,4] = 0.0
                batch_ys[k,5] = 0.0
                batch_ys[k,6] = 1.0                             
            else: 
        ...
        for dd in range(batch_xs.shape[0]):
            ...
            if normtc1  > 0.0025: #and batch_ys[dd,0]*5000 > 3*500*batch_ys[dd,1] and batch_ys[dd,0]*5000 < 20*500*batch_ys[dd,1]
