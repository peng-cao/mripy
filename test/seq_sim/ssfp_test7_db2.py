...
for _ in range(20):
    ...
    # tv regularization
    #tmp = 1j*np.zeros((nx,ny,4))
    #for i in range(4):
        #if i is 2:#apply tv penalty on b0 map
            #tmp[:,:,i] = pf.prox_tv2d( test_outy.reshape((nx,ny,4),order='F')[:,:,i], 0.05, step = 0.1 )
        #else: #apply l1 penalty on t1, t2, and pd maps
            #tmp[:,:,i] = pf.prox_l1_soft_thresh( test_outy.reshape((nx,ny,4),order='F')[:,:,i], 0.3)
            #tmp[:,:,i] = test_outy.reshape((nx,ny,4),order='F')[:,:,i]
        #ut.plotgray(np.absolute(tmp[:,:,i])
    #test_outy = np.absolute(tmp.reshape((nx*ny,4),order='F'))
