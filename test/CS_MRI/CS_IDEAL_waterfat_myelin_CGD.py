...
def test():
    ...
    #parameters
    xpar        = np.zeros((nx,ny,5), np.complex128)
    ...
    addx_myelin     = idealc.x_add_dx()
    addx_dfwaterfat = idealc.x_add_dx()
    addx_dfmyelin   = idealc.x_add_dx()
    ...
    addx_myelin.set_x     (xpar[...,2])
    addx_dfwaterfat.set_x (xpar[...,3])
    addx_dfmyelin.set_x   (xpar[...,4])
    ...
    Adwt_addx_m  = opts.joint2operators(tvop, addx_myelin)
    Adwt_addx_dfwatfat = opts.joint2operators(tvop, addx_dfwaterfat)
    Adwt_addx_dfmyelin = opts.joint2operators(tvop, addx_dfmyelin)
    ...
    def f(xi):
        ...
        + l1_r1 * alg.obj_sparsity(Adwt_addx_m, xi[...,2])
        + l1_r2 * alg.obj_sparsity(Adwt_addx_dfwatfat, xi[...,3])\
        + l1_r2 * alg.obj_sparsity(Adwt_addx_dfmyelin, xi[...,4])
    ...
    ut.plotim3(np.absolute(xpar)[...,0:3],bar=1)
    ut.plotim3(np.real(xpar + ostep*dxpar)[...,3],bar=1)
    ut.plotim3(np.imag(xpar + ostep*dxpar)[...,3],bar=1)
    ut.plotim3(np.real(xpar + ostep*dxpar)[...,4],bar=1)
    ut.plotim3(np.imag(xpar + ostep*dxpar)[...,4],bar=1)
