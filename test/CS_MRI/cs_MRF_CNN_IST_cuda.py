...
def tv_par( nx, ny, test_outy ):
    ...
    for i in range(4):
        tmp[:,:,i] = pf.prox_tv2d( test_outy.reshape((nx,ny,4),order='F')[:,:,i], 0.02, step = 0.1 )            
        #ut.plotim1((tmp[:,:,i]))
...
def wavel1_par( nx, ny, test_outy ):
    ...
    for i in range(4):
        tmp[:,:,i] = pf.prox_l1_Tf_soft_thresh2( dwt.backward, dwt.forward, \
            test_outy.reshape((nx,ny,4),order='F')[:,:,i], 0.05)
        #ut.plotim1((tmp[:,:,i]))
    test_outy = tmp.reshape((nx*ny,4),order='F')
    return test_outy
...
def constraints( test_outy, th = 0.001 ):
    ...
    for i in range(test_outy.shape[0]):
        if test_outy[i,3] < 0.1:
            test_outy[i,:] = np.zeros(4, np.float64)
    return test_outy
...
def test():
    ...
    ny            = 181
    # mask in ksp
    mask          = ut.mask3d( nx, ny, Nk, [15,15,0], 0.8)
    ...
    dtest_outy    = np.zeros((Nexample,4),dtype=np.float64)
    ...
    for _ in range(40):
        ...
        dtest_outy    = tmptest_outy - test_outy0 + 0.2*test_outy
        # gradient descent for test_outy
        acctest_outy  = acctest_outy - 0.1*dtest_outy
        print('gradient 0.5* d||f(p)-b||_2^2/dp: %g' % np.linalg.norm(dtest_outy))
        ...
        test_outy     = wavel1_par(nx, ny, acctest_outy) #  
